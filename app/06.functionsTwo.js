...
// ============================== 06 ====================================
/*
  Виконайте сортування всередині функції insertionSort. Тобто, є два цикли
  зовнішній, який проходиться по елементах масиву і внутрішній - ітерує по
  елементам, які порівнюються та переставляють їх у потрібному порядку. Таким
  чином здійснюється сортування через вставку елемета у правильне місце.
  Завдання реалізуйте внутрішній цикл, який виконує сортування кожного елемента
  вставкою на потрібне місце. Функція також повинна виводити елементи масиву
  один за одним.
*/
// Solution:
/*
  Ось псевдокод для алгоритму рішення виокористовуючи сортування вставкою
  for i = 1 to n-1:
    key = array[i]
    j = i-1
...
function insertionSort(arr) {
  // Зовнішній цикл: починаємо з другого елемента (індекс 1)
  // і рухаємося до кінця масиву
  for (let i = 1; i < arr.length; i++) {
    // Зберігаємо поточний елемент, який треба вставити у правильне місце
    let key = arr[i];

    // Індекс попереднього елемента
    let j = i - 1;

    // Внутрішній цикл: рухаємо всі елементи, які більші за key, вправо
    while (j >= 0 && arr[j] > key) {
      // Зсуваємо елемент на одну позицію вправо
      arr[j + 1] = arr[j];
      // Переходимо до наступного елемента ліворуч
      j--;
    }

    /*
      варіант вставки сортування від більшого до меншого - в низпадаючому порядку
      Внутрішній цикл: рухаємо всі елементи, які МЕНШІ за key, вправо
        // (бо ми хочемо від більшого до меншого)
        while (j >= 0 && arr[j] < key) {
            // Зсуваємо елемент на одну позицію вправо
            arr[j + 1] = arr[j];
            // Переходимо до наступного елемента ліворуч
  ...
  // Після завершення сортування друкуємо всі елементи масиву
  for (let i = 0; i < arr.length; i++) {
    console.log(arr[i]); // Виводимо кожен елемент з нового рядка
  }
}
// Tests:
console.log(insertionSort([5, 7, 2, 9, 1]));
console.log(insertionSort([5, 2, 9, 1, 5]));
/*
  Часова складність:
  Найкращий випадок: O(n)
  Коли масив вже відсортовано, сортування вставками виконує лише один
  прохід для підтвердження порядку сортування.
  Середній та найгірший випадки: O(n2)
  У середньому та найгіршому випадках це займає квадратичний час, оскільки
  для кожного елемента нам може знадобитися порівняти та зсунути всю
  відсортовану частину.
  Просторова складність: O(1)
  Сортування вставками – це алгоритм «на місці», тобто він не потребує
  додаткової пам'яті, пропорційної розміру вхідних даних. Простір, що 
  використовується для сортування, залишається постійним, незалежно від
