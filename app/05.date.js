...
const dayWord =
  /1$/.test(diffInDays) && !/11$/.test(diffInDays) ? "day" : "days";
...
// ======================================= 16 ===================================
/*
    Вам потрібно написати програму, яка відображає вітальне повідомлення на основі
    часу доби та використати у рішенні стрілкову функцію. При необхідності отримати
    поточну годину (0-23), використайте метод new Date().getHours(). Визначте
    стрілкову функцію з назвою getGreeting, яка приймає один аргумент, годину.
    Усередині функції використовуйте оператор if, щоб перевірити діапазон годин:
    Якщо година між 6 та 11 (ранок), поверніть "Доброго ранку!". Якщо година між 
    12 та 17 (день), поверніть "Доброго дня!". В іншому випадку поверніть "Доброго 
    вечора/ночі!".
*/
// Solution:
/*
    У завданні зазначаєсться про метод new Date().getHours(), але самі тести 
    насправді не запускають функцію з поточним часом. Натомість вони передають
    явні аргументи, такі як 10, 15 або 23. Це означає, що стрілочна функція
    повинна залежати лише від аргументу hour, а не викликати new Date() всередині.
    Можна використовувати поточний час у функції типу:
      console.log(getGreeting(new Date().getHours()));
    Важливо також зазначити, що метод new Date().getHours() повертає лише ціле 
    число години (0–23). Він не повертає хвилини. Тобто 
    якщо час 11:30, то метод new Date().getHours() повертає тільки годину -  11,
    що підпадає під ранок і привітання повинно бути Доброго ранку.
    Так само і для наприклад 17:20, new Date().getHours() → 17. Це денний час.
    Отже рішенням буде таке:
*/
...
console.log(getGreeting(23)); // Good evening/night!
console.log(getGreeting(0)); // Good evening/night!
console.log(getGreeting(new Date().getHours())); // Good ..

// Якщо в аргументах отримуємо і хвилини, то рішення таке
const getGreeting2 = (hour, minute = 0) => {
...
// Tests:
console.log(getGreeting2(11, 30)); // Good morning!
console.log(getGreeting2(18, 45)); // Good evening/night!

// ==================================================================
/*
  Напишіть програму, яка отримує натуральне число N з вхідних даних, що
  представляють номер місяця, та виводить кількість днів у цьому місяці.
  Ми припустимо, що рік не високосний, тому лютий має 28 днів.
*/
// Solution:
// Для цієї задачі не потрібно створювати об’єкт Date; достатньо просто
// визначити кількість днів за номером місяця, ось так:
let N = 8;
let days;

if ([1, 3, 5, 7, 8, 10, 12].includes(N)) {
  days = 31;
} else if ([4, 6, 9, 11].includes(N)) {
  days = 30;
} else if (N === 2) {
  days = 28;
} else {
  days = -1; // якщо номер місяця некоректний
}

console.log(days); // 31 день у серпні

// =============================== 09 ===================================
/*
  Дана дата може бути дійсною чи ні. Нам потрібно перевірити, чи місяць
  знаходиться між 1 та 12, чи це високосний рік (високосний рік - це кожен рік,
  який ділиться на 400, або ділиться на 4, але не на 100), і чи дні місяця
  точні, 30 та 31. Напишіть програму, яка для заданої дати виводить VALID,
  якщо це існуюча дата, та NOT VALID в іншому випадку.
  Вхідні дані:
    1 6 1992
  Вихідні дані: VALID
  Вхідні дані
    30 2 2004
  Вихідні дані: NOT VALID
  Пояснення: Перша дата є дійсною, але друга дата не є дійсною, оскільки лютий
  ніколи не має 30 днів.
*/
// Solution:
// Підключаємо модуль fs для читання вводу
// const fs = require("fs");
// Зчитуємо ввід і розділяємо по рядках
// const input = fs.readFileSync(0, "utf8").trim().split(" "); або

const input9 = ["30", "2", "2004"]; // Зчитуємо день, місяць та рік

// Отримуємо день, місяць і рік
const day = Number(input9[0]);
const month = Number(input9[1]);
const year = Number(input9[2]);
...
// Масив із кількістю днів у кожному місяці
...
if (month < 1 || month > 12) {
  ...
  // День має бути від 1 до максимальної кількості днів у цьому місяці
